...
namespace Windows.UI.Xaml.Controls
{
	public abstract partial class VirtualizingPanelLayout : DependencyObject
	{
		...
		internal Size MeasureOverride(Size availableSize)
		{
			...
			{
				Console.WriteLine("Measured without an ItemsControl: simply return size(0,0) for now...");
				return new Size(0, 0);
			}
		...
		internal Size ArrangeOverride(Size finalSize)
		{
			if (ItemsControl == null)
			{
				Console.WriteLine("Arranged without an ItemsControl: simply return size(0,0) for now...");
				return new Size(0, 0);
			}
